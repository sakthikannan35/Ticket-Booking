...
class KannanCini:
    def __init__(self):
        self.movies = ['garudan', 'Maharaja', 'Thriller','Vidamuyarchi']
        self.classes = {"first class": 150, "second class": 200}
        self.db = mysql.connector.connect(
            host="localhost",
            user="root",
            password="Root@123",
            database="kannan_theater"
        )
        self.cursor = self.db.cursor()

    def create_database(self):
        self.cursor.execute("CREATE DATABASE IF NOT EXISTS kannan_theater")
        self.db.commit()
    ...
    def send_email(self, bill, total):
        try:
            ...
            s.sendmail("sakthikannan305@gmail.com", bill, msg)
            s.quit()
...
if __name__ == "__main__":
    kannan_cinimas = KannanCini()
    kannan_cinimas.create_database()
    kannan_cinimas.create_table()
    while True:
        command = input("Type 'book' to book a ticket or 'exit' to exit: ")
        if command.lower() == 'exit':
            break
        elif command.lower() == 'book':
            kannan_cinimas.book_ticket()
        else:
